/**
 * Audits the *published* supply-chain surface of `@nest-native/asyncapi`.
 *
 ...
 * peer the samples pull in) — none of which can reach a consumer. `npm audit
...
const npmCache = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-asyncapi-audit-cache-'));
const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-asyncapi-audit-consumer-'));
...
try {
  ...
  npm(['run', 'build', '--workspace', '@nest-native/asyncapi'], { cwd: repoRoot, stdio: 'inherit' });

  const packOutput = npm(['pack', '--json', '--workspace', '@nest-native/asyncapi'], {
    cwd: repoRoot,
  });
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: 'nest-native-asyncapi-audit-consumer', version: '0.0.0', private: true }, null, 2)}\n`,
  );
